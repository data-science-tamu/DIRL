classdef Maintenance_real_reward_v3_EXPECT_COST_gpu_complexIP < rl.env.MATLABEnvironment& Abs_Maintenance_env
...
%         state2
    ...
    methods              
        ...
        function this = Maintenance_real_reward_v3_EXPECT_COST_gpu_complexIP(param, unit, th, cap_w, noworkload, unboundedAgent, agentUseDiscrete, strictPenalty, addGroupVar, noWorkloadState, incentivized, show_every_rewards, discretize_states, show_progress,cellInfo)
...
% end
        ...
        function [Observation, Reward, IsDone, LoggedSignals] = step(this,Action)
            if(this.inspectionEverytime)
                inspect_action = ones(1,length(single(Action)));
                Action = cat(2,single(Action),inspect_action);
...
%                 fprintf('Failure cost = %.4f  \n',failure_cost);
           ...
           %Cal throughput after maintenance----------------------------
           thrput_ws_maintenance  = zeros(process_num,1);
           thrput_ws_inspection  = zeros(process_num,1);
           thrput_ws_incentivefailure = zeros(process_num,1);
           thrput_ws_truefailure  = zeros(process_num,1);
           run_component_id = 1;
           for i= 1:process_num
               num_comp_per_process = length(actual_cap{i});
               perf_rate_mt = ones(1,num_comp_per_process);
               perf_rate_ins = ones(1,num_comp_per_process);
               perf_rate_infl = ones(1,num_comp_per_process);
               perf_rate_trfl = ones(1,num_comp_per_process);
               for j = 1:num_comp_per_process 
                    if replace(run_component_id+j-1) > 0
                        perf_rate_mt(j) = (1-this.proportionLossTPMaintenance);
                    end
                    if inspect(run_component_id+j-1) > 0
                        perf_rate_ins(j) = (1-this.proportionLossTPInspection);
                    end
               ...
               thrput_ws_maintenance(i) = sum(actual_cap{i} .* perf_rate_mt);
               thrput_ws_inspection(i) = sum(actual_cap{i} .* perf_rate_ins);
               thrput_ws_incentivefailure(i) = sum(actual_cap{i} .* perf_rate_infl);
               thrput_ws_truefailure(i) = sum(actual_cap{i} .* perf_rate_trfl);
               % Update index
               run_component_id = run_component_id + num_comp_per_process;
           end
           loss_thrput_maintenance = this.max_cap - min(thrput_ws_maintenance);
           loss_thrput_inspect = this.max_cap - min(thrput_ws_inspection) ;
           ...
           cost_obj = loss_thrput_maintenance *this.cost_Lossthroughput+ ...
                loss_thrput_inspect *  this.cost_Lossthroughput + ...
                loss_thrput_incentivefailure *  this.cost_Lossthroughput + ...
                sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
                failure_cost;
    
               
           cost_actual = loss_thrput_maintenance * this.cost_Lossthroughput+ ...
                loss_thrput_inspect *  this.cost_Lossthroughput + ...
                loss_thrput_truefailure *  this.cost_Lossthroughput + ...
                sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
                   sum(failure.*this.failure_cost.*this.dt); % Failure Cos
