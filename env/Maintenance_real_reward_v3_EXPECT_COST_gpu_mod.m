classdef Maintenance_real_reward_v3_EXPECT_COST_gpu_mod < rl.env.MATLABEnvironment& Abs_Maintenance_env
    ...
    properties
        ...
        add_incentive = true;
        random_init_degrad = false;
        random_init_degrad_t0 = false;
...
%         state2
    ...
    methods              
        ...
        function this = Maintenance_real_reward_v3_EXPECT_COST_gpu_mod(param, unit, th, cap_w, noworkload, unboundedAgent, agentUseDiscrete, strictPenalty, addGroupVar, noWorkloadState, incentivized, show_every_rewards, discretize_states, show_progress)
...
%             state = this.State;
            if this.random_init_degrad
                random_init_state = rand(1)*0.5*this.threshold;
...
%             workload = this.getWorkloadAction(Action);
            ...
            for i=1:this.unit
                if replace(i)==1
                    this.event_record = [this.event_record;[this.State(1:this.unit),1,i]];
                    this.replace_mc(i)
                end
            end
...
%             fprintf('Current mean2 is %.2f and variance2 is %.4f \n',this.mean_unit2,this.var_unit2)
            ...
            for i= 1:this.unit
                if failure(i) == 1
                    this.event_record = [this.event_record;[this.State(1:this.unit),0,i]];
                    this.replace_mc(i);
