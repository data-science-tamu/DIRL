classdef Maintenance_real_reward_v3_EXPECT_COST_gpu_complexThrput < rl.env.MATLABEnvironment& Abs_Maintenance_env
...
%         state = this.reset()
        ...
        cost_Lossthroughput 
        failureToThroughput
        % cost_LossthroughputInspect
        proportionLossTPInspection
        proportionLossTPMaintenance
        count_fail =0;
        count_maintenance =0;
        count_inspection = 0;
        fixedMaintenacetoCost
...
%         state2
    ...
    methods              
        ...
        function this = Maintenance_real_reward_v3_EXPECT_COST_gpu_complexThrput(param, unit, th, cap_w, noworkload, unboundedAgent, agentUseDiscrete, strictPenalty, addGroupVar, noWorkloadState, incentivized, show_every_rewards, discretize_states, show_progress,cellInfo)
            
%             this.unit = unit;
            
            if(nargin>4 && noworkload)
                if (cellInfo.inspectionAction)
                    if(addGroupVar)
                        if (cellInfo.inspectionEverytime)
                            action_space_dim = single(unit+1); 
                        else
                            action_space_dim = single(2*(unit+1));  %Inspection
                        end
                    else
                        if (cellInfo.inspectionEverytime)
                            action_space_dim = single(unit); 
                        else
                            action_space_dim = single(2*unit);  %Inspection
                        end
                    end
                else
                    if(addGroupVar)
                        action_space_dim = single(unit+1);  %Inspection
                    else
                        action_space_dim = single(unit); 
                    end
                end
                ...
                if(addGroupVar)
                    action_space_dim = single(2*(2*unit+1)); % if noworkload=1, unit+1, else, 2unit +1
                else
...
%             end
            ...
            % this.cost_LossthroughputInspect = cellInfo.lossTPInspection_cost;
            this.inspectionEverytime = cellInfo.inspectionEverytime;
            this.proportionLossTPInspection = cellInfo.proportionLossTPInspection;
            this.proportionLossTPMaintenance = cellInfo.proportionLossTPMaintenance;
            this.inspectionAction = cellInfo.inspectionAction;
            this.failureToThroughput = cellInfo.failureToThroughput;
            this.fixedMaintenacetoCost = cellInfo.fixedMaintenacetoCost ;
            this.fixedCostToInitiateMaintenace = cellInfo.fixedCostToInitiateMaintenace ;
...
%             fprintf('At step %d : The new beta is %7.2f \n' ,this.time ,this.sample_beta(idx_mc));
            ...
            this.lifetime_mc(idx_mc) = 0;
            this.ts_after_inspection(idx_mc) = 0;
            this.observable_state(idx_mc) = 0;
...
%             state = this.State;
        ...
        function inspect = getInspectionAction(this, Action)
            if(this.addGroupVar)
                do_inspection = single(Action(2*(this.unit+1))>this.th_replace);
                inspect = (Action((this.unit+2):(2*this.unit+1))>this.th_replace) .* do_inspection;
            else
                do_inspection = single(true);
                inspect = (Action((this.unit+1):(2*this.unit))>this.th_replace) .* do_inspection;
            end
        end
...
%                 end
            ...
            if(this.inspectionAction)
                if(this.inspectionEverytime)
                    inspect_temp =ones(1,length(replace));
                else
                    inspect_temp = this.getInspectionAction(Action);
                end
            else
                inspect_temp =ones(1,length(replace));
            end
            inspect = (1-replace).*inspect_temp;
...
%             fprintf('lifetime_mc is %d \n',this.lifetime_mc)
            ...
            states_current = degradation.*(1-failure);
            if(this.inspectionAction)
                for i=1:this.unit
                    if inspect(i)==1
                        this.ts_after_inspection(i) = 0;
                        this.observable_state(i) = states_current(i);
                    else
                        this.ts_after_inspection(i) = this.ts_after_inspection(i)+1;
                    end
                end 
            else
                this.ts_after_inspection = zeros(1,this.unit);
                this.observable_state = states_current;
            end
            
            % Update variables based on the observed value
            this.mean_unit2 = (this.var_unit0.*this.observable_state+this.var_meas_err.*this.mean_unit0)./(this.var_unit0.*(this.lifetime_mc-this.ts_after_inspection)+this.var_meas_err);
            this.var_unit2 = (this.var_unit0.*this.var_meas_err)./(this.var_unit0.*(this.lifetime_mc-this.ts_after_inspection)+this.var_meas_err);
...
%             Observation = this.State;
            ...
            [Reward, reward_actual] = this.getReward_without_workload(failure, replace, inspect );
...
%             Reward = Reward - this.getPenalty(Action);
            ...
            notifyEnvUpdated(this);
             ...
             %% Cost function
            function [cost_obj, cost_actual] = getCost_without_workload(this, failure, replace, inspect)

            % Update mean_fu and var_fu based on inspection decision
            d_time = this.ts_after_inspection +1;
            mean_fu = this.observable_state + this.mean_unit2.*d_time;
            try
                var_fu = ((d_time).^2 .* this.var_unit2) + (d_time .* this.var_meas_err) + 1e-5 ; % To help convergence
            catch err
                disp(err)
            end
...
%                 fprintf('Failure cost = %.4f  \n',failure_cost);
           ...
           % 
           % %Cal throughput after maintenance
           % cap_per_process  = zeros(process_num,1);
           % run_component_id = 1;
           % for i= 1:process_num
           ...
           %% Version 1: Inspection reflects in cost -.- -.- -.- -.- -.- -.-
           %{
            %Cal throughput after inspection
           cap_per_process  = zeros(process_num,1);
           run_component_id = 1;
           for i= 1:process_num
               num_comp_per_process = length(actual_cap{i});
               noninspection_decision = abs(1-inspect(run_component_id:(run_component_id+num_comp_per_process-1)));
               actual_cap_per_process = actual_cap{i} .* noninspection_decision;
               cap_per_process(i) = sum( actual_cap_per_process);
               run_component_id = run_component_id + num_comp_per_process;
           end
           actual_thrput = min(cap_per_process);
           loss_thrput_inspect = this.max_cap - actual_thrput ;
           
           cost_obj = loss_thrput *this.cost_Lossthroughput+ ...
                loss_thrput_inspect * this.cost_LossthroughputInspect + ...
                sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
                failure_cost;
           ...
           cost_actual = loss_thrput * this.cost_Lossthroughput+ ...
                ...
                sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
                   sum(failure.*this.failure_cost.*this.dt); % Failure Cost
           %}
           %% Version 2 : Inspection reflects in throughput  -.- -.- -.- -.- -.- -.-
           %  %Cal throughput after inspection
           % cap_per_process  = zeros(process_num,1);
           % run_component_id = 1;
           % for i= 1:process_num
           %     num_comp_per_process = length(actual_cap{i});
           %     performance_rate = ones(1,num_comp_per_process);
           %     for j = 1:num_comp_per_process 
           %          if inspect(run_component_id+j-1) > 0
           %              performance_rate(j) = (1-this.proportionLossTPInspection);
           %          end
           %     end
           %     actual_cap_per_process = actual_cap{i} .* performance_rate;
           %     cap_per_process(i) = sum( actual_cap_per_process);
           %     run_component_id = run_component_id + num_comp_per_process;
           % end
           % actual_thrput = min(cap_per_process);
           % loss_thrput_inspect = this.max_cap - actual_thrput ;
           %  cost_obj = loss_thrput *this.cost_Lossthroughput+ ...
           %      loss_thrput_inspect *  this.cost_Lossthroughput + ...
           %      sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
           %      failure_cost;
           % 
           % 
           ...
           %      loss_thrput_inspect *  this.cost_Lossthroughput + ...
           %      sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
           %         sum(failure.*this.failure_cost.*this.dt); % Failure Cos

           %% Thoroughput loss reflected into maintenance and failure cost
           %Throughput part --------------------------------------------
           actual_cap = this.throughput;
           process_num = length(actual_cap);
           prob_failure = (1-normcdf(-((mean_fu-this.threshold)./sqrt(var_fu)), 0, 1));

           %Cal throughput after maintenance----------------------------
           thrput_ws_incentivefailure = zeros(process_num,1);
           thrput_ws_truefailure  = zeros(process_num,1);
           run_component_id = 1;
           for i= 1:process_num
               num_comp_per_process = length(actual_cap{i});
               perf_rate_infl = ones(1,num_comp_per_process);
               perf_rate_trfl = ones(1,num_comp_per_process);
               for j = 1:num_comp_per_process 
                    if replace(run_component_id+j-1) > 0
                        perf_rate_infl(j) = (1-this.proportionLossTPMaintenance);
                        perf_rate_trfl(j) = (1-this.proportionLossTPMaintenance);
                        this.count_maintenance = this.count_maintenance+1;
                    else
                        if(this.inspectionAction) && (inspect(run_component_id+j-1) > 0)
                            perf_rate_infl(j) = (1-this.proportionLossTPInspection);
                            perf_rate_trfl(j) = (1-this.proportionLossTPInspection);
                            this.count_inspection = this.count_inspection+1;
                        end
                        if this.failureToThroughput
                            if failure(run_component_id+j-1) > 0
                                perf_rate_trfl(j) = 0;
                                this.count_fail = this.count_fail+1;
                            end
                            probfail_incentive = 1-prob_failure(run_component_id+j-1);
                            if probfail_incentive > this.proportionLossTPInspection
                                perf_rate_infl(j) = 1-prob_failure(run_component_id+j-1);
                            end
                        end
                    end
               end
               thrput_ws_incentivefailure(i) = sum(actual_cap{i} .* perf_rate_infl);
               thrput_ws_truefailure(i) = sum(actual_cap{i} .* perf_rate_trfl);
               % Update index
               run_component_id = run_component_id + num_comp_per_process;
           end
           loss_thrput_truefailure = this.max_cap - min(thrput_ws_truefailure) ;
           if(this.add_incentive)
              loss_thrput_incentivefailure = this.max_cap - min(thrput_ws_incentivefailure);
           else
              loss_thrput_incentivefailure =  loss_thrput_truefailure ;
           end
           ...
           %     for j = 1:num_comp_per_process 
           %          if replace(run_component_id+j-1) > 0
           %              performance_rate(j) = (1-this.proportionLossTPMaintenance);
           %          end
           %     end
           %     actual_cap_per_process = actual_cap{i} .* performance_rate;
           %     cap_per_process(i) = sum( actual_cap_per_process);
           ...
           % end
           % actual_thrput = min(cap_per_process);
           % loss_thrput_maintenance = this.max_cap - actual_thrput ;
           % 
           % %Cal throughput after inspection----------------------------
           % cap_per_process  = zeros(process_num,1);
           % run_component_id = 1;
           ...
           cost_obj = loss_thrput_incentivefailure * this.cost_Lossthroughput + ...
                sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
                failure_cost;
    
               
           cost_actual = loss_thrput_truefailure * this.cost_Lossthroughput + ...
                sum(this.replace_cost.*replace)+...      % Replace cost : sum individual
                   sum(failure.*this.failure_cost.*this.dt); % Failure Cos

           if this.fixedMaintenacetoCost 
            cost_obj = cost_obj +  sign(sum(replace))*this.fixedCostToInitiateMaintenace ;
            cost_actual = cost_actual +  sign(sum(replace))*this.fixedCostToInitiateMaintenace ;
           end
...
%             end
            if (idx>this.num_training +  this.num_test)
                if(idx>this.num_training + 2* this.num_test)
                    fprintf('WARNING: SOMETHING IS WRONG: env save count went above the needed steps.');
                else
                    fprintf('WARNING: THIS IS TESTING TWO TIMES');
                end
            end
...
%             fprintf('The initial beta is %7.2f \n' , this.sample_beta);
            ...
            this.ts_after_inspection = zeros(1,this.unit);
            this.observable_state = zeros(1,this.unit);
...
%             this.ActionInfo.Elements = this.MaxForce*[-1 1];
        ...
        % Reward function
        function [reward, reward_actual] = getReward_without_workload(this, failure, replace, inspect)
%             [cost, cost_actual] = this.getCost(failure, workload, replace);
            [cost, cost_actual] = this.getCost_without_workload(failure, replace, inspect);
