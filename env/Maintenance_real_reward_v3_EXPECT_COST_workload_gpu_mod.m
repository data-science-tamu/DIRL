classdef Maintenance_real_reward_v3_EXPECT_COST_workload_gpu_mod < rl.env.MATLABEnvironment& Abs_Maintenance_env
...
%         state2
    ...
    methods              
        ...
        function this = Maintenance_real_reward_v3_EXPECT_COST_workload_gpu_mod(param, unit, th, cap_w, noworkload, unboundedAgent, agentUseDiscrete, strictPenalty, addGroupVar, noWorkloadState, incentivized, show_every_rewards, discretize_states, show_progress)
...
%             this.unit = unit;
            if(nargin>4 && noworkload)
                ...
                if(addGroupVar)
                    ...
                    action_space_dim = single(2*unit); % if noworkload=1, unit+1, else, 2unit +1
...
%             this.lifetime_mc = this.lifetime_mc + 1;
            ...
            % Update Variables : Previous version
...
%             this.var_unit2 = (this.var_unit0.*this.var_meas_err)./(this.var_unit0.*this.get_state_cum_w_vec()+this.var_meas_err);
            ...
            this.set_state_cum_w_vec( (this.get_state_cum_w_vec() + this.dt * workload).*(1-failure) )
            this.mean_unit2 = (this.var_unit0.*degradation+this.var_meas_err.*this.mean_unit0)./(this.var_unit0.* this.dt *this.get_state_cum_w_vec()+this.var_meas_err);
            this.var_unit2 = (this.var_unit0.*this.var_meas_err)./(this.var_unit0.* this.dt *this.get_state_cum_w_vec()+this.var_meas_err);
...
%         var_fu = np.multiply(np.square(workload*self.dt), self.var_unit2) + np.multiply(workload*self.dt, self.var_meas_err) + 1e-5 # To help convergence
% 
...
%             this.var_unit2 = zeros(1,this.unit) * this.var_beta;
            ...
            this.sample_beta = abs(normrnd(this.mean_beta, sqrt(this.var_beta), [1,this.unit]));
            this.mean_unit2 = ones(1,this.unit) * this.mean_beta;
            this.var_unit2 = ones(1,this.unit) * this.var_beta;
